...
class PascalVOCDetectionEvaluator(DatasetEvaluator):
        ...
        for iou, recall in recalls.items():
            ...
            for cls_id, rec in enumerate(recall):
                if cls_id in range(self.num_seen_classes) and len(rec) > 0:
                    ...
                    # print("in wi:")
                    # print(range(len(rec)))
                    # print(lambda i: abs(rec[i] - recall_level))
                    # print(len(rec))
                    # print(index)
                    # input()
    ...
    def evaluate(self):
        ...
        with tempfile.TemporaryDirectory(prefix="pascal_voc_eval_") as dirname:
            ...
            # print("in class names:")
            # print(self._class_names)
            # input()

            # for thresh in range(50, 100, 5):
            # thresh = 50
            # unknown_recall_and_precision(res_file_template,
            ...
            for cls_id, cls_name in enumerate(self._class_names):
                ...
                # for thresh in range(50, 100, 5):
                thresh = 50
                rec, prec, ap, unk_det_as_known, num_unk, tp_plus_fp_closed_set, fp_open_set, pre_unk_count, bg_unk_count = voc_eval(
                    res_file_template,
                    self._anno_file_template,
                    self._image_set_path,
                    cls_name,
        ...
        global unknown_is_detected
        global unknown_pred_correct
        total_num_unk = num_unks[50][0]
        self._logger.info("unknown num:{}, unknown recall num:{}, unknown right num:{}, detected percent:{}, correct percent:{}".format(total_num_unk, unknown_is_detected, unknown_pred_correct, float(unknown_is_detected)/total_num_unk*100, float(unknown_pred_correct)/unknown_is_detected*100))
        self._logger.info("predict unknown num:" + str(pre_unk_count))
        self._logger.info("background to unknown misclassification num:" + str(bg_unk_count))
        self._logger.info("misclassification rate:" + str(float(bg_unk_count)/pre_unk_count*100))
...
unknown_class_recs_flag = {}
unknown_is_detected = 0
unknown_pred_correct = 0
...
def voc_eval(detpath, annopath, imagesetfile, classname, ovthresh=0.5, use_07_metric=False, known_classes=None):
    ...
    # init unknown_class_recs
    global unknown_class_recs_flag

    if len(unknown_class_recs_flag) == 0:
        # init global unknown flag
        for imagename in imagenames:
            ...
            # difficult = np.array([False for x in R]).astype(np.bool)  # treat all "difficult" as GT
            det = [False] * len(R)
            unknown_class_recs_flag[imagename] = {"bbox": bbox, "difficult": difficult, "det": det}
    ...
    if classname=='unknown':
        ...
        # print("len(sorted_ind):",len(sorted_ind))
        print("nd:",nd)
        # print("len(BB):",len(BB))
        bg_count = 0
        for d in range(nd):
            R = all_recs[image_ids[d]]
            ...
            if BBGT.size > 0:
                ...
                if ovmax<=0.3:
                    bg_count+=1
        print('---------------')
        print('bg numbers:',bg_count)
        print("bg ration:",bg_count/nd)
        print('---------------')
    ...
    for d in range(nd):
        ...
        # print("in class iou computation:")
        # print(BBGT.shape)
        # print(bb.shape)
        # input()
        ...
        if ovmax > ovthresh:
            if not R["difficult"][jmax]:
                if not R["det"][jmax]: # avoid over detection
                    tp[d] = 1.0
                    R["det"][jmax] = 1
                    if classname == "unknown":
                        global unknown_pred_correct
                        unknown_pred_correct += 1
                else:
                    fp[d] = 1.0
        else:
            fp[d] = 1.0
        ...
        # for computing unknown recall
        R_unk = unknown_class_recs_flag[image_ids[d]]
        ...
        if BBGT_unk.size > 0:
            ...
            # print("in unknown iou computation")
            # print(overlaps.shape)
            # print(ovmax_unk)
            # print(jmax)
            # input()
        ...
        if ovmax_unk > ovthresh:
            if not R_unk["difficult"][jmax]:
                if not R_unk["det"][jmax]: # avoid over detection
                    unknown_is_detected += 1
                    R_unk["det"][jmax] = True
                    # print("global flag is changed:")
                    # print(unknown_class_recs_flag[image_ids[d]])
                    # input()
    ...
    return rec, prec, ap, is_unk_sum, n_unk, tp_plus_fp_closed_set, fp_open_set, nd, bg_count
